...
// ... (Sisa fungsi showToast, updatePanelState, updateArcana, renderSpectatorHUD SAMA SEPERTI SEBELUMNYA) ...
// ... Pastikan fungsi window.joinGame juga tetap ada ...
...
function updatePanelState(state) {
  ...
  if (state.phase === "loading") {
    ...
    if (elGameOver) elGameOver.classList.add("hidden");
    if (btnRematch) { btnRematch.disabled = false; btnRematch.innerText = "REMATCH"; }
    ...
    if (elGameOver) {
      ...
      if (elGoTitle && elGoMsg) {
        if (isWinner) { elGoTitle.innerText = "VICTORY"; elGoTitle.style.color = "#00FF00"; elGoMsg.innerText = "The enemy King has fallen!"; } 
        else { elGoTitle.innerText = "DEFEAT"; elGoTitle.style.color = "#FF0000"; elGoMsg.innerText = "Your King has fallen..."; }
      }
      ...
      } else { elRematchStatus.classList.add("hidden"); }
    }
  }
}
...
function updateArcana(state) {
  ...
  elFill.style.background = current >= MAX_ARCANA ? "linear-gradient(90deg, #e040fb, #ffffff)" : "linear-gradient(90deg, #7b1fa2, #e040fb)";
}
...
function renderSpectatorHUD(state) {
  ...
  if (currentSignature === lastSpectatorSignature) return;
  lastSpectatorSignature = currentSignature;
...
window.joinGame = (teamId) => { if (_socket) { _socket.emit("request_join_game", teamId); showToast("Requesting to join...", "info"); } };