  ...
  if (unit.attackCooldown <= 0) {
    ...
    if (isRanged) {
      ...
      if (unit.aoeRadius > 0) {
        ...
        // [FIX] USE DYNAMIC TARGET RULE (ENEMY/ALLY/BOTH)
        // Ambil targetRule dari unit (default 'enemy')
        const targetRule = unit.targetRule || 'enemy'; // Pastikan properti ini ada di unit.js
        
        // Tentukan Origin Ledakan
        ...
        dealAreaDamage(
            ...
            unit.team,
            unit.targetHeight,
            unit.targetTeam // <--- Pass 'both' here if unit configured so
        );
