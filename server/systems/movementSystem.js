...
export function updateMovement(gameState, dt) {
  ...
  for (const unit of units) {
    ...
    // ... SEPARATION CODE ...
    const currentSeparation = unit.isCrossing ? SEPARATION_FORCE * 0.2 : SEPARATION_FORCE;
    let separationX = 0, separationY = 0, neighbors = 0;
    for (const other of units) {
      if (unit === other) continue;
      const dist = distance(unit, other);
      if (dist < UNIT_RADIUS * 2) {
        let pushX = unit.col - other.col; let pushY = unit.row - other.row;
        const len = Math.sqrt(pushX * pushX + pushY * pushY);
        if (len > 0) { separationX += pushX / len; separationY += pushY / len; neighbors++; }
      }
    }
    if (neighbors > 0) { moveX += (separationX/neighbors)*currentSeparation; moveY += (separationY/neighbors)*currentSeparation; }

    // ... BUILDING COLLISION CODE ...
    if (unit.movementType !== 'flying') {
        for (const b of buildings) {
          if (b.hp <= 0) continue;
          const dx = unit.col - b.col; const dy = unit.row - b.row;
          const dist = Math.sqrt(dx * dx + dy * dy);
          const minDist = UNIT_RADIUS + BUILDING_RADIUS;
          if (dist < minDist) {
            const pushFactor = (minDist - dist) / dist;
            moveX += dx * pushFactor * 5.0; moveY += dy * pushFactor * 5.0;
          }
        }
    }
    ...
    if (unit.state === "moving") {
        ...
        if (!finalDest) {
            ...
            if (unit.targetTeam === 'ally') {
                // Healer Default: Follow Leader / Retreat
                 let leader = null; let minD = Infinity;
                for (const other of units) {
                    if (other.id === unit.id || other.team !== unit.team || other.targetTeam === 'ally') continue;
                    const d = distance(unit, other); if (d < minD) { minD = d; leader = other; }
                }
                if (leader) { finalDest = { col: leader.col, row: leader.row }; stopDistance = 2.0; }
                else {
                    let bestB = null; let minBD = Infinity;
                    for (const b of buildings) { if (b.team !== unit.team || b.hp <= 0) continue; const d = distance(unit, b); if (d < minBD) { minBD = d; bestB = b; } }
                    if (bestB) { finalDest = { col: bestB.col, row: bestB.row }; stopDistance = 2.5; }
                }
            } else {
                // Attacker Default: Cari Tower Musuh
                let bestB = null; let minBD = Infinity;
                for (const b of buildings) { if (b.team === unit.team || b.hp <= 0) continue; const d = distance(unit, b); if (d < minBD) { minBD = d; bestB = b; } }
                if (bestB) { finalDest = { col: bestB.col, row: bestB.row }; }
                else { finalDest = { col: 9, row: unit.team === 0 ? GRID.rows : 0 }; }
            }
        }
        ...
        if (finalDest) {
            ...
            const isFinalLeg = (waypoint.col === finalDest.col && waypoint.row === finalDest.row);
    ...
    if (unit.movementType === 'ground') {
        ...
        if (inRiverZone) {
            const onBridgeLeft = Math.abs(nextCol - BRIDGE_COLUMNS[0]) < 1.0;
            const onBridgeRight = Math.abs(nextCol - BRIDGE_COLUMNS[1]) < 1.0;
            if (!onBridgeLeft && !onBridgeRight) {
...
function getSmartWaypoint(unit, finalDest) {
    ...
    const unitY = unit.row; const destY = finalDest.row;
    const unitSide = unitY < RIVER_TOP_BANK ? "top" : unitY > RIVER_BOT_BANK ? "bottom" : "river";
    const destSide = destY < RIVER_TOP_BANK ? "top" : destY > RIVER_BOT_BANK ? "bottom" : "river";
    if (unitSide === destSide || unitSide === "river" || destSide === "river") return finalDest;
    ...
    const distToRiverEdge = Math.min(Math.abs(unit.row - RIVER_TOP_BANK), Math.abs(unit.row - RIVER_BOT_BANK));

    if (distToBridgeX > 3.0 && distToRiverEdge < 1.5) {
      const safetyRow = unitSide === "top" ? RIVER_TOP_BANK - 2.0 : RIVER_BOT_BANK + 2.0;
      return { col: bridgeCol, row: safetyRow };
    }
    let bridgeDestRow;
    if (destSide === "top") bridgeDestRow = RIVER_TOP_BANK - 1.0;
    else bridgeDestRow = RIVER_BOT_BANK + 1.0;
